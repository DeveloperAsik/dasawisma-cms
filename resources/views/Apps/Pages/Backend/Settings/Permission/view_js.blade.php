<script>
    var fnInitTree = function (id, logged, name) {
        $.ajax({
            type: "GET",
            url: base_developer_url + ('settings/menu/get_menu/' + id + '/' + logged + '/' + name),
            dataType: "json",
    ...
    var Index = function () {
        return {
            ...
            init: function () {
                ...
                $('.btn').on('click', function () {
                    ...
                    switch (cls) {
                        ...
                        case 'opt_edit':
                            ...
                            var uri = base_developer_url + 'settings/menu/get_data/';
                            $.ajax({
                                url: uri,
                        ...
                        case 'opt_remove':
                            bootbox.confirm("Are you sure?", function (result) {
                                if (result) {
                                    ...
                                    var uri = base_developer_url + 'settings/menu/remove/';
                                    $.ajax({
                                        url: uri,
                                        method: "POST", //First change type to method here
                                        data: formdata,
                                        success: function (response) {
                                            fnToStr('Successfully remove exist data');
                        ...
                        case 'opt_delete':
                            bootbox.confirm("Are you sure?", function (result) {
                                if (result) {
                                    ...
                                    var uri = base_developer_url + 'settings/menu/delete/';
                                    $.ajax({
                                        url: uri,
                                        method: "POST", //First change type to method here
                                        data: formdata,
                                        success: function (response) {
                                            fnToStr('Successfully remove exist data', 'success');
                                            $('#tree_' + module_id_exist).treeview('remove');
                                            fnInitTree(module_id_exist, 1, module_name_exist);
                                            fnCloseModal();
                                        },
                                        error: function (response) {
                                            fnToStr('Failed remove exist data', 'error');
                                            $('#tree_' + module_id_exist).treeview('remove');
                                        }
                                    });
                                }
                ...
                $('form#add_edit').on('submit', function (e) {
                    e.preventDefault();
                    bootbox.confirm("Are you sure?", function (result) {
                        if (result == true) {
                            ...
                            var action = 'insert new data';
                            var urisubmit = base_developer_url + 'settings/menu/insert/';
                            if (id) {
                                action = 'update existing data';
                                urisubmit = base_developer_url + 'settings/menu/update/';
                                var formdatasubmit = {
                                    id: Base64.encode(id),
                                    name: $('input[name="name"]').val(),
                                    parent_id: parent_id,
                            ...
                            $.ajax({
                                ...
                                data: formdatasubmit,
                                success: function (response) {
                                    fnToStr('Successfully ' + action, 'success');
                                    $('#tree_' + module).treeview('remove');
                                    fnInitTree(module, 1, module_name_exist);
                                    fnCloseModal();
                                },
                                error: function () {
                                    fnToStr('Successfully ' + action, 'error');
                                    $('#tree_' + module).treeview('remove');
                                    fnInitTree(module, 1, module_name_exist);
                                    fnCloseModal();
                                }
